...
class MessageApi extends Api{
	...
	public function index() {
        ...
        foreach($list as $k=>$v){
            $info = current($v['to_user_info']);
            $list[$k]['to_user_info'] = $info ? $info : (object)array();
        }
		$this->exitJson($list);
	}
    ...
    public function detail(){
        $messageid = intval( $this->data['message_id'] );
	...
	public function loadMessage() {
        ...
        model('Message')->setMessageIsRead(intval($_REQUEST['list_id']), $this->mid, 0);
		// 临时解决方案
...
// 			}
	...
	public function doPost() {
		...
		if ( !CheckPermission('core_normal','send_message') ) {
			$this->exitJson( array() ,10038,'你还没有权限发送私信哦');
		}
		...
		$_POST['content'] = filter_keyword(h($_REQUEST['content']));
		// 图片附件信息
		if (trim(t($_POST['attach_ids'])) != '') {
			$_POST['attach_ids'] = explode('|', $_POST['attach_ids']);
			$_POST['attach_ids'] = array_filter($_POST['attach_ids']);
