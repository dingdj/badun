...
class AlbumApi extends Api{
    ...
    */
    ...
    public function getAlbumList(){
        ...
        if ($tag_id > 0) {
            $inSql = "SELECT row_id FROM ".C('DB_PREFIX')."app_tag WHERE app='classroom' AND `table`='album' AND tag_id={$tag_id}";
            $where .= " AND id IN($inSql)";
        }
        
        $data = $this->album->where($where)->field('id,uid,album_title,album_order_count
,album_intro,cover,album_score')->order($order)->limit($this->_limit())->select();
        if( !$data ) {
        	$this->exitJson( array() );
        }
        //计算总课程
    ...
    public function getCatalog(){
        ...
        $sql = 'SELECT `id`,`video_title` FROM ' . C("DB_PREFIX") . 'zy_video WHERE `id` IN (' . (string) $videos . ') and is_del=0 ORDER BY find_in_set(id,"' . (string) $videos . '")';
        $result = M('')->query($sql);
