...
class LoginHooks extends Hooks {
    ...
    private static $validLogin = array(
            ...
            "weixin"    => array("wx_app_id", "wx_app_secret"),
    ...
    //可同步发布动态的站点
    private static $validPublish = array('sina', 'qq', 'qzone','weixin');//, 'renren'); 暂时关闭人人网同步-不知道哪里抽风审核不过
    //应用名称
    ...
    public function login_input_footer($param) {
        ...
        if ($regInfo ['register_type'] != 'admin' && ! empty ( $platform )) {
            ...
            foreach ( $platform as $key => $value ) {
                $html .= sprintf ( "<dd><a href=\"%s\" class=\"icon-%s\"></a></dd>", $value, $key );
            }
            $html .= '</dl>';
            $html .= '</div>';
            echo $html;
  ...
  /*  //添加验证代码meta标签
    ...
    public function account_bind_after($param){
        ...
        foreach ( $validAlias as $k=>$v ) {
            ...
            //$icon = $this->htmlPath.'/html/image/icon_'.$k.'.gif';
            $is_bind = false;
            $is_sync = false;
    ...
    public function login_bind_publish_weibo($param){
        ...
        if($type == 'weixin'){
            header("Location: ".U('public/Passport/authentication_wx'));
        }
        $platform = new $type();
    ...
    public function no_register_display($param) {
        ...
        if ($_GET ['do'] == "bind") {
            ...
            $result['jumpUrl'] = U('classroom/User/setInfo', array('tab'=>4));
            $result['isAdmin'] = 1;
        //当前操作如果是登录
        } else {
            $type = t ( $_GET ['type'] );
