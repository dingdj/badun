...
class PassportModel {
	...
	private function _recordLogin($uid, $is_remember_me = false) {
		...
		$map['browser_ver'] = getBrowserVer();
		$map['os'] = getOs();
		$map['place'] = getRegion();
		$map['locktime'] = 0;

        $credit = M('credit_setting')->where(array('id'=>1,'is_open'=>1))->field('id,name,score,count')->find();
        if($credit['score'] > 0){
            $ltype = 6;
            $note = '每日签到获得的积分';
        }
        model('Credit')->addUserCreditRule($_SESSION['mid'],$ltype,$credit['id'],$credit['name'],$credit['score'],$credit['count'],$note);
	...
	public function isValidPhone($phone){
		...
		return preg_match("/^13[0-9]{1}[0-9]{8}$|15[0189]{1}[0-9]{8}$|18[0-9]{1}[0-9]{8}$/",$mobilephone) !==0;
	}
	/**
	 * 加密函数
