...
class UserModel extends Model {
	...
	protected $fields = array (
            ...
            46 => 'os',
            47 => 'place',
            48 => 'ouschool_buyer_time',
            49 => 'ouschool_buyer_num',
			'_autoinc' => true,
			'_pk' => 'uid'
	);
	...
	public function getUserList($limit = 20, $map = array(), $order = "uid DESC") {
		...
		if (isset ( $_POST )) {
			...
			if (! $_POST ['uid']) {
				...
				//机构用户筛选
				if (! empty ( $_POST ['mhm_title'] )) {
					$map['mhm_id'] = intval($_POST ['mhm_title']);
				}
			}
		}
	...
	public function getUserExamList($limit = 20, $map = array(), $order = "uid DESC ") {
		$tp = C('DB_PREFIX');
		// 添加用户表的查询，用于关联查询
		if (isset ( $_POST )) {
			$_POST ['uid'] && $map ['uid'] = intval ( $_POST ['uid'] );
		...
		$list =$this->join("{$tp}ex_user_exam ON user_id=uid")->where($where)->order ( $order )->group("uid")->findPage ( $limit );
		// 数据组装
		$userGroupHash = array ();
		$uids = array ();
		foreach ( $list ['data'] as $k => $v ) {
	...
	public function addUser($user) {
		...
		if (! $result) {
			...
			}else if(! empty ( $user ['user_group'] )){
				model ( 'UserGroupLink' )->domoveUsergroup ( $result, $user ['user_group'] );
			}
			// 添加用户职业关联信息
			if (! empty ( $_POST ['user_category'] )) {
	...
	public function trueDeleteUsers($ids) {
		...
		$result = $this->where ( $map )->save ( array('is_del'=>1) );
		$this->cleanCache ( $uid_array );
		if (! $result) {
	...
	private function _getUserInfo($map, $field = "*") {
		...
		if (! $user) {
			...
			$user ['space_url'] = ! empty ( $user ['domain'] ) ? U ( 'classroom/UserShow/index', array (
					'uid' => $user ["domain"] 
			) ) : U ( 'classroom/UserShow/index', array (
					'uid' => $user ["uid"] 
			) );
    ...
    public function add($data='',$options=array()){
		...
		$data['browser_ver'] = getBrowserVer();
		$data['os'] = getOs();
		$data['place'] = getRegion();

        $result = parent::add($data,$options);

        if($result){
            // 获取系统配置 给用户添加初始积分
            $register['score'] = model('Xdata')->get('admin_Config:register')['default_user_score'];
            $register['uid'] = $result;
            $register['ltime'] = time();
            M('credit_user') -> add($register);

            $learncModel = D('ZyLearnc', 'classroom');
            $splitModel  = D('ZySplit', 'classroom');
            if(method_exists($learncModel, 'initUser') && method_exists($splitModel, 'initUser')){
                $learncModel->initUser($result, null, true);
                $splitModel ->initUser($result, null, true);
            }
        }
        return $result;
    }
	 ...
	 * @return array 指定用户的相关信息
	 */
	public function findUserInfo($uid,$field){
		$uid = intval($uid);
		if ($uid <= 0) {
			$this->error = L('PUBLIC_UID_INDEX_ILLEAGAL'); // UID参数值不合法
			return false;
		}
		$userInfo = $this->where(array('uid'=>$uid))->field($field)->find();
		return $userInfo;
	}
	 ...
	 * 			显示的字段
	 *
	 * @return array 指定用户的学习记录
	 */
	public function findUserLearnInfo(){
		$uid = intval($this->mid);
		if ($uid <= 0) {
			$this->error = L('PUBLIC_UID_INDEX_ILLEAGAL'); // UID参数值不合法
			return false;
		}
        $field = 'vid,sid,time,ctime';
        $order = 'ctime desc';
