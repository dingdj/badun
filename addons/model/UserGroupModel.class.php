...
class UserGroupModel extends Model {
	...
	protected $fields = array(0=>'user_group_id',1=>'user_group_name',2=>'ctime',3=>'user_group_icon',4=>'user_group_type',5=>'app_name',6=>'is_authenticate','school','specialty','address','s_address','other_data');
	 ...
	 * @param unknown_type $uid
	 */
	public function isSchool( $uid ){
		$uid = model( 'UserGroupLink' )->where('uid='.$uid.' and user_group_id=4')->getField('uid');
		if($uid){
			$sid = model('School')->getSchooldStrByMap(array('uid'=>$uid),'id');
	 ...
	 * @param unknown_type $uid
	 */
	public function isPinclass( $uid ){
		$uid = model( 'UserGroupLink' )->where('uid='.$uid.' and user_group_id=5')->getField('uid');
		if($uid){
			$sid = M('user')->where('uid = '.$uid)->getField('mhm_id');
            return $sid ? $sid : false;
        }
        return false;
	}
	...
	public function isTeacher( $uid ){
		$uid = model( 'UserGroupLink' )->where('uid='.$uid.' and user_group_id=3')->getField('uid');
