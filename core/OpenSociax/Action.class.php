	...
	private function initSite() {
        ...
        foreach ($currency_category as $key => $val){
            foreach($val['child'] as &$v){
                $v['nav_left'] = M('zy_currency_category')->where(['pid'=>$v['id'],'is_nav_left'=>1])->field('zy_currency_category_id,title')->limit(4)->select();
            }
            $currency_category[$key]['school_data'] = M('school')->where(array('fullcategorypath'=>array('like',"%,{$val['id']},%"),
                'is_del'=>0,'status'=>1,'is_re'=>1))->field("id,title,cover,doadmin")->limit(2)->findAll();
            $currency_category[$key]['vdeo_data']   = M('zy_video')->where(array('is_mount'=>1,'fullcategorypath'=>array('like',"%,{$val['id']},%"),
                'is_del'=>0,'is_activity'=>1,'uctime'=>array('gt',time()),'listingtime'=>array('lt',time())))->field("id,video_title,cover,type")
                ->limit(3)->findAll();
...
//            || !$school_info['doadmin']
        ...
        $this->assign('currency_category', $currency_category);
