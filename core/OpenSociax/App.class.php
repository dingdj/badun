...
class App
{
    ...
    static public function execApp() {
        ...
        //防止CSRF，排出支付异步通知
        if(APP_NAME != 'classroom' && MODULE_NAME != 'Pay'){
            preg_match('/http(s)?:\/\/(.*)\.(.*\..*)/',SITE_URL,$matchs);
            if(strtoupper($_SERVER['REQUEST_METHOD'])=='POST' && stripos($_SERVER['HTTP_REFERER'],'.'.$matchs[3]) === false && $_SERVER['HTTP_USER_AGENT'] !== 'Shockwave Flash') {
                die('illegal request.');
            }
        }
...
//            if ( APP_NAME == 'classroom' && !in_array(strtolower(MODULE_NAME), $publicAccess) && strtolower(ACTION_NAME) != 'message' && isMobile() && in_array('wap', C('DEFAULT_APPS')) ) {
//                // 智能手机到3G版
