...
function getAlbumNameForID($vid) {
    static $names = array();
    if (!isset($names[$vid])) {
        $names[$vid] = D('Album')->where(array('id' => $vid))->getField('album_title');
    }
    return $names[$vid];
}
...
function isGetResource($type, $resid, $resource = array()) {
    ...
    foreach ($resource as $value) {
        switch ($value) {
            case 'video':
                if ($type == 1) {
                    $album_video = M('Album')->where(array('id' => intval($resid)))->getField('album_video');
...
function getVideoMergeNum($uid, $tmp_id) {
    return D('ZyVideoMerge' , 'classroom')->getNum($uid, $tmp_id);
}
