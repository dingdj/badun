...
class VideoSectionModel extends Model
{
	...
	private function _MakeTree($pid, $vid ,$level = 0)
	{
		...
		if($result) {
			foreach($result as $key => $value) {
				...
				$list[$id]['vid'] = $value['vid'];
				$list[$id]['cid'] = $value['cid'];
				$list[$id]['eid'] = $value['eid'];
				$list[$id]['wid'] = $value['wid'];
				$list[$id]['title'] = $value['title'];
				$list[$id]['v_title'] = M('zy_video_data')->where('status=1 and is_del=0 and id='.$value['cid'])->getField('title');
                $list[$id]['duration'] = M('zy_video_data')->where('status=1 and is_del=0 and id='.$value['cid'])->getField('duration');
	...
	{
		// 判断是否有重复的值
		$isExist = $this->_model->where("pid={$pid} AND title='{$title}' AND vid='{$extra['vid']}'")->count();
		if($isExist != 0) {
			return false;
		}
		// 添加分类操作
