...
class ZyServiceModel {
     ...
     * 4:购买记录/订单，添加失败
     */
    public function buyOnlineVideo($uid, $video_id,$ext_data = array()) {
        if ($this->checkVideoAccess($uid, $video_id)) {
            return 1;
        }
        $time = time();
...
//        }
        ...
        $video_spilt_status = $this->addVideoSplit($id,$uid,$video,$ext_data['price'] ? : $prices['price']);
...
//            }
        ...
        $video_spilt_status = $this->addLiveSplit($id,$uid,$live_info,$ext_data['price'] ? : $prices);
     ...
     */
    public function addLiveSplit($order_id,$uid,$live_info = array(),$prices){
        //找不到直播课程
        if (!$live_info){
            return 2;
        }
        ...
        //如果分享者有且不为平台管理员、课程所属机构管理员和教师，   //购买用户相应机构管理员、&& $share_uid != $oschool_uid
        ...
        if ($share_uid && $share_uid != 1  && $share_uid != $school_uid && $share_uid != $teacher_uid) {
            ...
            $data['share_sum'] = round($school_num * $proportion['sp_share'], 2);//平台分享者的金额

            $data['sid'] = $school_uid;//课程所属机构分成的管理员用户id
            $data['school_sum'] = round($school_num * $proportion['sp_school'], 2);//课程所属机构获得的金额
        }else{
            $data['sid'] = $school_uid;//课程所属机构分成的管理员用户id
     ...
     * 13:创建订单明细流水失败
     */
    public function addVideoSplit($order_id,$uid,$video = array(),$prices){
        //找不到课程
        if (!$video){
            return 2;
        }
...
//        }
        ...
        //扣掉平台手续费后的总金额
        $school_num = $prices * $proportion['pac_school'];//机构及分享者的总金额

        //如果购买用户在所属机构抽成有效期/抽成次数还有 参与所属机构分成
        $ouschool_buyer = model('User')->where(['uid'=>$uid])->field('ouschool_buyer_time,ouschool_buyer_num')->find();
        if($ouschool_buyer['ouschool_buyer_time'] > time() && $ouschool_buyer['ouschool_buyer_num'] > 0){
            $data['oschool_uid']        = $oschool_uid;//购买用户机构与课程所属机构分成比例-购买用户机构用户id
            $data['ouats_ouschool_sum'] = round($school_num * $proportion['ouats_buyschool'],2);//购买用户机构与课程所属机构分成比例-购买用户机构用户的金额

            $school_num = $school_num * $proportion['ouats_theschool'];//课程机构及分享者的总金额
        }
        ...
        if ($share_uid && $share_uid != 1 && $share_uid != $school_uid && $share_uid != $teacher_uid) {
            ...
            $data['share_sum'] = round($school_num * $proportion['sp_share'], 2);//平台分享者的金额
            $data['sid'] = $school_uid;//课程所属机构分成的管理员用户id
            $data['school_sum'] = round($school_num * $proportion['sp_school'], 2);//平台分享者的金额//课程所属机构获得的金额
        }else{
            $data['sid'] = $school_uid;//课程所属机构分成的管理员用户id
            $data['school_sum'] = $school_num;//课程所属机构获得的金额
        }
