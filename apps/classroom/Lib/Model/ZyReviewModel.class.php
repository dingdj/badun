...
class ZyReviewModel extends Model
{
	...
	public function getReviewList($limit,$map = array(), $order = "id DESC"){
		if (isset ( $_POST )) {
			...
			$_POST ['star'] && $map ['star'] = intval ( $_POST ['star'] * 20 );
     ...
     * @name 计算好评率
     */
    public function getCommentRate($type = 1,$oid = 0){
        $map['parent_id'] = array('eq',0);
		$map['type'] = array('eq',$type);
		$map['oid']  = array('eq',$oid);
        ...
        if($count == 0){
            ...
            $star_all = $this->where($map)->sum('star');
            return round(($star_all / ($count*100)) * 100) . '%';
        }
