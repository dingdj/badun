...
class AdminUserCardAction extends AdministratorAction{
    ...
    public function addCard(){
        ...
        if($id){
            $this->savePostUrl = U ( 'classroom/AdminUserCard/doUserCard','type=save&id='.$id);
			...
			$this->assign('pageTitle','添加优惠券');
            $this->savePostUrl = U ('classroom/AdminUserCard/doUserCard','type=add');
            //说明是添加
    ...
    public function getSubCategory(){
        ...
        $coupon = $_POST['coupon'];
        $time  = time();
        $map = "status=1 AND is_del=0 AND end_time>$time AND coupon_type=0 ";
        if($type>0) {
            switch ($type) {
                case 1:
                    $field = "price";
    ...
    public function doUserCard(){
        ...
        $sid = model('Coupon')->where('id='.$_POST['cid'])->getField('sid');
