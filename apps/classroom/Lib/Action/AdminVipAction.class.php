...
class AdminVipAction extends AdministratorAction{
    ...
    public function index(){
        $this->pageKeyList = array( 'id','title','sort','vip_year','cover','is_del','DOACTION' );
        $this->_initTabSpecial();
        $this->pageButton[] = array('title'=>'删除','onclick'=>"admin.delVipAll('vip等级','AdminVip')");
        $this->assign('pageTitle','vip等级管理');
        $data = M('user_vip')->order("sort DESC")->findPage(20);
        foreach($data['data'] as &$val){
			...
			if($val['is_del'] == 0) {
				$val['DOACTION'] .= " | <a href=javascript:admin.closeVip(" . $val['id'] . ",'vip等级','AdminVip');>禁用</a>";
				$val['is_del'] = "<span style='color: green;'>正常</span>";
			}else{
				$val['DOACTION'] .= " | <a href=javascript:admin.openVip(" . $val['id'] . ",'vip等级','AdminVip');>启用</a>";
...
//			$val['DOACTION'] .= ' | <a onclick="admin.delVip('.$val['id'].',\'delVip\');" href="javascript:;" title="此操作会彻底删除数据">彻底删除</a>';
    ...
    public function addVip(){
    	$id = $_GET['id'];
    	$this->_initTabSpecial();
    	$this->pageKeyList = array ( 'title','sort','vip_year','cover');
    	$this->notEmpty = array ( 'title','sort','vip_year','cover' );
    	if($id){
    		$this->pageKeyList = array ('title','sort','vip_year','cover');
    		$res = M('user_vip')->where( 'id=' .$id )->find();
			$this->savePostUrl = U ( 'classroom/AdminVip/doAddVip',array('id'=>$id));
    		$this->assign('pageTitle','编辑vip等级-'.$res['title']);
    		//说明是编辑
