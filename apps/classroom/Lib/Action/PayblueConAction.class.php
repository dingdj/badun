...
class PayblueConAction extends CommonAction{
    ...
    public function operatConOrder()
    {
        ...
        $blueconprice = t($_POST['blueconprice']);
        $blueconnums = t($_POST['blueconnums']);
        $starttime = t($_POST['nowtime']);
        $money = floatval($blueconprice* $blueconnums);

        $title = t($_POST['title']).date("YmdH",$starttime)."-".date("YmdH",$starttime+3600)."时段的绿色通道并发数";
        if ($money <= 0) {
            $this->error('金额不能为负数');
        }
        if ($_POST) ;
        {
            $bluecon = M('concurrent') -> where('id = 1') -> getField('blueconprice');
            $old_price = floatval($bluecon * $blueconnums);
            ...
            $data['blueconnums'] = $blueconnums;
            $data['price'] = $money;
            $data['starttime'] = $starttime-1;
            $data['endtime'] = $starttime + 3601;
            $data['ctime'] = time();
            $data['uid'] = $this ->mid;
            $data['pay_status'] = 1;
            $data['rel_id'] = $id;
            $res = M('zy_order_bluecon')-> add($data);

            if($res){
                if ($_POST['pay'] == 'alipay') {
                    $this->alipay(array(
                        'starttime' => $starttime,
                        'blueconnums' => $blueconnums,
                        'out_trade_no' => $id,
                        'total_fee' => $money,
                        'subject' => "{$this->site['site_keyword']}-购买{$title}",
                    ));
    ...
    protected function alipay($args){
        ...
        $alipayClass->setConfig(array(
            ...
            "notify_url"      => 'http://'.strip_tags($_SERVER['HTTP_HOST']).'/alipayBlueConAnsy.html',
            "return_url"      => U('classroom/PayblueCon/aliru'),//页面跳转同步通知页面路径
        ));

        $alipayClass->addData('extra_common_param',json_encode(array('starttime'=>$args['starttime'],'blueconnums'=>$args['blueconnums'],'total_fee'=>$args['total_fee'])));
     ...
     * @packages public
     */
    public function alipayBlueConAnsy(){
        ...
        $starttime      = intval($extra_common_param['starttime']);
        $blueconnums    = $extra_common_param['blueconnums'];
        ...
        if ($trade_status == 'TRADE_FINISHED') {
            ...
            if ($result) {
                ...
                $pay_status = M('zy_order_bluecon')->where(array('uid'=>$this_uid,'starttime'=>$starttime-1,'ctime'=>$ctime))->getField('pay_status');

                if($pay_status == 3){
                    echo '购买成功';
                }else{
                    $con_order_info = $this->buyOperating($starttime, $out_trade_no, $blueconnums, $total_fee,$this_uid);
                    if ($con_order_info == 1) {
                        echo '购买成功';
                    } else {
                        echo '购买失败';
            ...
            if ($result) {
                $this_uid = $re->where('id = '.$out_trade_no)->getField('uid');
                $ctime = $re->where('id = '.$out_trade_no)->getField('ctime');
                $pay_status = M('zy_order_bluecon')->where(array('uid'=>$this_uid,'starttime'=>$starttime,'ctime'=>$ctime))->getField('pay_status');
    ...
    public function  buyOperating($starttime,$out_trade_no,$blueconnums,$price,$this_uid){
        ...
        $res = M('zy_order_bluecon')->where(array('uid'=>$this_uid,'starttime'=>$starttime-1,'ctime'=>$ctime))->save($data);
    ...
    protected function wxpay($data){
        ...
        if($data){
            ...
            $attr  = json_encode(array('starttime'=>$data['starttime'],'blueconnums'=>$data['blueconnums'],'total_fee'=>$data['total_fee']));
...
//            D('ZyRecharge')->add($da);
    ...
    public function getPayStatus(){
        ...
        if($data['status'] == 1){
            ...
            $order_info = $this->buyOperating($attach['starttime'],$data['id'],$attach['blueconnums'],$attach['total_fee']);
            if($order_info == 1){
                $info = '购买成功!';
            }else{
                $info = '购买失败';
