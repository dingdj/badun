...
class PublicAction extends CommonAction
{
    ...
    public function followyou(){
        ...
        if($type == 'off')
        {
            $map['uid'] = $uid ;
            $map['fid'] = $fid ;
            $map['tid'] =intval($_POST['tid']);
            $rest = M('user_follow') -> where($map) ->delete();
            if($rest){
                $credit = M('credit_setting')->where(array('id'=>54,'is_open'=>1))->field('id,name,score,count')->find();
                if($credit['score'] < 0){
                    $ptype = 7;
                    $note = '取消关注讲师扣除的积分';
                }
                model('Credit')->addUserCreditRule($this->mid,$ptype,$credit['id'],$credit['name'],$credit['score'],$credit['count'],$note);

                $this->mzSuccess("取消关注成功");
            }else{
        ...
        if(0 == $follow_state['following']) {
            ...
            if($result) {
                ...
                $credit = M('credit_setting')->where(array('id'=>28,'is_open'=>1))->field('id,name,score,count')->find();
                if($credit['score'] > 0){
                    $ptype = 6;
                    $note = '关注讲师获得的积分';
                }
        ...
        $res=model('Follow')->doFollow($this->mid,$fid,$tid);
        if($res){
	...
	public function doreviewvote(){
        ...
        $cuid = M('zy_comment')->where('id='.$kzid)->getField('uid');
		if($type){
			//取消投票
	...
	public function collect(){
        ...
        if($sctype == 9)
        {
            $data['source_table_name'] = 'zy_topic';
        }
		...
		if(!$type){
			...
			if($i === false){
                ...
                if($is_video == 1){
                    $credit = M('credit_setting')->where(array('id'=>3,'is_open'=>1))->field('id,name,score,count')->find();
                    if($credit['score'] > 0){
                        $ctype = 6;
                        $note = '收藏课程获得的积分';
                    }
                }else if($is_video == 2){
                    $credit = M('credit_setting')->where(array('id'=>11,'is_open'=>1))->field('id,name,score,count')->find();
                    if($credit['score'] > 0){
                        $ctype = 6;
                        $note = '收藏直播获得的积分';
                    }
