...
class AdminVideoMountAction  extends AdministratorAction
{
    ...
    public function index(){
        $this->pageKeyList = array('id','video_title','cover','v_price','t_price','user_title','mhm_name','teacher_name',
            'video_collect_count','video_comment_count','video_question_count','video_note_count','video_score',
            'video_order_count','mount_status','best','atime','DOACTION');
        $this->pageButton[] =  array('title'=>'搜索','onclick'=>"admin.fold('search_form')");
        $this->searchKey = array('id','video_title','uid','teacher_id','mhm_id');
        $this->searchPostUrl = U('classroom/AdminVideoMount/index',array('tabHash'=>'index'));
    ...
    public function action(){
        ...
        $this->pageButton[] =  array('title'=>'搜索','onclick'=>"admin.fold('search_form')");
        $this->searchKey = array('id','video_title','uid','teacher_id','mhm_id');
        $this->searchPostUrl = U('classroom/AdminVideoMount/action',array('tabHash'=>'index'));
        $school = model('School')->getAllSchol('','id,title');
        $this->opt['mhm_id'] = $school;
...
//            $value['DOACTION'] = '<a href=" '.U('classroom/AdminVideoMount/lesson',array('vid'=>$value['id'])).' ">课时管理</a> | ';
//            $value['DOACTION'] .= '<a href=" '.U('classroom/AdminVideoMount/learn',array('uid'=>$value['uid'],'id'=>$value['id'])).' ">学习记录</a> | ';
//            //获取章节
...
//            $value['DOACTION'] .= '<a href="'.U('classroom/AdminVideoMount/askVideo',array('tabHash'=>'askVideo','id'=>$value['id'])).'">提问</a> | ';
//            $value['DOACTION'] .= '<a href="'.U('classroom/AdminVideoMount/noteVideo',array('tabHash'=>'noteVideo','id'=>$value['id'])).'">笔记</a> | ';
//            $value['DOACTION'] .= '<a href="'.U('classroom/AdminVideoMount/reviewVideo',array('tabHash'=>'reviewVideo','id'=>$value['id'])).'">评价</a> | ';
//            if( $value['is_del'] == 0 && $value['is_activity'] == '0'){
//                $value['DOACTION'] .= '<a href="javascript:void();" onclick="admin.crossVideo('.$value['id'].',true)">通过审核</a> | ';
//            }
//            $value['DOACTION'] .='<a href="'.U('classroom/AdminVideoMount/addVideo',array('id'=>$value['id'],'tabHash'=>'editVideo')).'">编辑</a> |';
//
//            if($value['is_del'] == 1) {
