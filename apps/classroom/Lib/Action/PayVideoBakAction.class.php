...
//        explodeoad(join(DIRECTORY_SEPARATOR, array(SITE_PATH, 'api','pay','alipay_v2','AlipayNotify.php')));
//        joinoad(join(DIRECTORY_SEPARATOR, array(SITE_PATH, 'api','pay','alipay_v2','AlipayNotify.php')));

        $alipayClass = new \Alipay($alipay_config);
        //设置支付的Data信息
        $extra_common_param_w3g = json_encode(array('vid'=>$args['vid'],'vtype'=>$args['vtype'],'coupon_id'=>$args['coupon_id']));
        if($this->is_wap){
            $alipayClass->setConfig(array(
                ...
                "notify_url"      => 'http://'.strip_tags($_SERVER['HTTP_HOST']).'/alipay_alinu.html',
                "return_url"      => 'http://'.strip_tags($_SERVER['HTTP_HOST'])."/alipay_aliru.html?extra_common_param_w3g={$extra_common_param_w3g}"
            ));
        }else{
            $alipayClass->setConfig(array(
                "out_trade_no"  => $args['out_trade_no'].'h'.date('YmdHis',time()).mt_rand(1000,9999),//商户网站订单系统中唯一订单号，必填
                ...
                "_input_charset"    => trim(strtolower($alipay_config['input_charset'])),
                "notify_url"      => 'http://'.strip_tags($_SERVER['HTTP_HOST']).'/alipay_alinu.html',
                "return_url"      => 'http://'.strip_tags($_SERVER['HTTP_HOST']).'/alipay_aliru.html'
            ));
        }

        $alipayClass->addData('extra_common_param',json_encode(array('vid'=>$args['vid'],'vtype'=>$args['vtype'],'coupon_id'=>$args['coupon_id'])));

        if($this->is_wap){
            $res = $alipayClass->goAliService('h5');
        }else{
            //调用阿里的服务,默认调用PC端支付
            $res = $alipayClass->goAliService('pc');
        }
        echo $res;exit;
    }
