...
class PayConAction extends CommonAction{
    ...
    public function operatConOrder()
    {
        ...
        if ($_POST) ;
        {
            ...
            $title = t($_POST['title'])."{$this->site['site_keyword']}平台并发数";

            if ($money <= 0) {
                $this->error('请选择或填写购买金额');
            }
            $con_num = t($_POST['con_num']);
            if (t($_POST['month']) == "onemonth") {
                $onemoney = M('concurrent')->where('id = 1')->getField('onemprice');
                $old_price = $onemoney * $con_num;
                $data['etime'] = strtotime("+1 month");
            }
            if (t($_POST['month']) == "threemonth") {
                $onemoney = M('concurrent')->where('id = 1')->getField('threemprcie');
                $old_price = $onemoney * $con_num;
                $data['etime'] = strtotime("+3 month");

            }
            if (t($_POST['month']) == "sixmonth") {
                $onemoney = M('concurrent')->where('id = 1')->getField('sixmprice');
                $old_price = $onemoney * $con_num;
                $data['etime'] =  strtotime("+6 month");

            }
            if (t($_POST['month']) == "oneyear") {
                $onemoney = M('concurrent')->where('id = 1')->getField('oneyprice');
                $old_price = $onemoney * $con_num;
                $data['etime'] =  strtotime("+1 year");
    ...
    protected function alipay($args){
        ...
        $alipayClass->setConfig(array(
            ...
            "notify_url"      => 'http://'.strip_tags($_SERVER['HTTP_HOST']).'/alipayConAnsy.html',
            "return_url"      => U('classroom/PayCon/aliru'),//页面跳转同步通知页面路径
        ));

        $alipayClass->addData('extra_common_param',json_encode(array('month'=>$args['month'],'connums'=>$args['connums'],'total_fee'=>$args['total_fee'])));
    ...
    public function alipayConAnsy(){
        ...
        $month      = intval($extra_common_param['month']);
        ...
        if ($trade_status == 'TRADE_FINISHED') {
            ...
            if ($result) {
                ...
                $pay_status = M('zy_order_concurrent')->where(array('uid'=>$this_uid,'connums'=>$connums,'rel_id'=>$out_trade_no))->getField('pay_status');

                if($pay_status == 3){
                    echo '购买成功';
                }else{
                    $con_order_info = $this->buyOperating($connums,$out_trade_no,$total_fee,$this_uid);
                    if ($con_order_info == 1) {
                        echo '购买成功';
                    } else {
                        echo '购买失败';
    ...
    public function  buyOperating($connums, $out_trade_no,$total_fee,$this_uid){
        ...
        $res = M('zy_order_concurrent')->where(array('uid'=>$this_uid,'rel_id'=>$out_trade_no))->save($data);
    ...
    protected function wxpay($data){
        ...
        if($data){
            ...
            $attr  = json_encode(array('month'=>$data['month'],'connums'=>$data['connums'],'total_fee'=>$data['total_fee']));
...
//            D('ZyRecharge')->add($da);
    ...
    public function getPayStatus(){
        ...
        if($data['status'] == 1){
            ...
            $order_info = $this->buyOperating($attach['connums'],$data['id'],$attach['total_fee']);
            if($order_info == 1){
                $info = '购买成功!';
            }else{
                $info = '购买失败';
