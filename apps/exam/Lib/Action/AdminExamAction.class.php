...
class AdminExamAction extends AdministratorAction
{
    ...
    public function index()
    {
        ...
        $this->pageKeyList             = array('exam_id', 'exam_category', 'exam_name', 'exam_count', 'exam_status', 'exam_begin_time', 'exam_end_time', 'exam_passing_grade', 'exam_publish_result_flg', 'exam_times_mode', 'sort', 'DOACTION');
        $this->pageButton[]            = array('title' => '搜索考试管理', 'onclick' => "admin.fold('search_form')");
        $this->searchKey               = array('exam_id', 'exam_name', 'exam_category', 'exam_begin_time', 'exam_end_time');
        $this->searchPostUrl           = U('exam/AdminExam/index');
        $category                      = M('ex_exam_category')->where('pid=0')->getField('ex_exam_category_id,title');
        $this->opt['exam_category']    = $category;
        $this->opt['exam_category'][0] = '不限'; //必须放在赋值的下面
        $this->_listpk = 'exam_id';
    ...
    public function postRecycle()
    {
        $this->_initExamListAdminMenu();
        $this->_initExamListAdminTitle();
        $this->pageKeyList  = array('exam_id', 'exam_category', 'exam_name', 'exam_begin_time', 'exam_end_time', 'exam_passing_grade', 'exam_publish_result_flg', 'exam_times_mode', 'exam_admin', 'DOACTION');
    ...
    public function addExam()
    {
        ...
        if ($_GET["id"]) {
            ...
            $exam["exam_publish_result_tm"] = $exam["exam_publish_result_tm"] ? date("Y-m-d H:i:s", $exam["exam_publish_result_tm"]) : "";
            $exam["exam_user_signup_time"]  = $exam["exam_user_signup_time"] ? date("Y-m-d H:i:s", $exam["exam_user_signup_time"]) : "";
            $exam["exam_user_signup_end"]   = $exam["exam_user_signup_end"] ? date("Y-m-d H:i:s", $exam["exam_user_signup_end"]) : "";
            $this->assign("exam", $exam);

            $paper_list = M('ex_paper')->where('paper_id=' . $exam['paper_id'])->findAll();
            $this->assign("paper_list", $paper_list);
        } else {
            $this->pageTitle['addExam'] = '添加考试信息';
        }
        //$exam_category = M('ex_exam_category')->findAll();
        $paper_lists   = M("ex_paper")->where('paper_status=1 and paper_is_del=0')->findAll();
        foreach ($paper_lists as &$value) {
            $value['paper_category'] = M('ex_paper_category')->where('paper_category_id=' . $value['paper_category'])->getField('paper_category_name');
        }

       // $this->assign("exam_category", $exam_category);
        $this->assign("paper_lists", $paper_lists);
    ...
    public function doAddExam()
    {
        ...
        $post['ex_exam_category_idhidden'] = rtrim($post['ex_exam_category_idhidden'],',0');
        $data['fullcategorypath'] = ','.$post['ex_exam_category_idhidden'].',';
        if (strpos($post['ex_exam_category_idhidden'], ',') !== false) {
            $data['exam_categoryid']      = substr($post['ex_exam_category_idhidden'],strripos($post['ex_exam_category_idhidden'],',')+1);
        } else {
            $data['exam_categoryid'] = $post['ex_exam_category_idhidden'];
        }
        $data["paper_id"]                    = $post["paper_id"];
        $data["exam_status"]                 = $post["exam_status"];
        $data["exam_publish_result_flg"]     = $post["exam_publish_result_flg"] ? $post["exam_publish_result_flg"] : 0;
        $data["exam_passing_again_flg"]      = $post["exam_passing_again_flg"];
        $data["exam_publish_result_tm_mode"] = $post["exam_publish_result_tm_mode"];
        $data["exam_user_signup_flg"]        = $post["exam_user_signup_flg"] ? $post["exam_user_signup_flg"] : 0;
    ...
    private function _getData($limit = 20, $is_del)
    {
        ...
        if (isset($_POST)) {
            ...
            if ($_POST['exam_category']) {
                $map['exam_category_id'] = intval($_POST['exam_category']);
            }
        }
        $map['exam_is_del'] = $is_del;
        $list               = M("ex_exam e")->join("`{$tp}ex_exam_category` c ON e.exam_categoryid = c.ex_exam_category_id")->where($map)->order('exam_update_date desc')->field('exam_id,c.title as exam_category_name,exam_name,exam_status,exam_begin_time,exam_is_del,exam_end_time,exam_passing_grade,exam_publish_result_flg,exam_times_mode,exam_admin,exam_is_del,e.sort')->findPage($limit);
        foreach ($list['data'] as $key => $value) {
            ...
            $list['data'][$key]['exam_name']               = $value['exam_name'];
    ...
    public function putRecycle()
    {
        ...
        $where = array(
            'exam_id' => array('in', $ids),
        );
        $data['exam_is_del'] = 1;
        $res                 = M('ex_exam')->where($where)->save($data);
    ...
    {
        $map["paper_id"]     = $_POST["exam_paper_id"];
        $map["paper_status"] = 1;
        $map["paper_is_del"] = 0;
        $res                 = M('ex_paper')->where($map)->find();
        if ($res) {
            exit(json_encode(array('status' => '1', 'data' => $res)));
        } else {
            exit(json_encode(array('status' => '0', 'data' => '没有找到该试卷')));
        }
    }
    /**
     * 删除考试信息
    ...
    public function ExamDetil()
    {
        ...
        foreach ($user_exam['data'] as $key => $value) {
            $user_exam['data'][$key]['user_name']= getUserSpace($value['user_id'], null, '_blank');;
    ...
    public function doExport()
    {
        ...
        /* sheet标题 */
        $objActSheet->setTitle($exam_info["exam_name"] . "-考试信息");
        //合并单元格
        $objActSheet->mergeCells('A1:G1');
        $objActSheet->setCellValue('A1', $exam_info["exam_name"] . "-考试信息");
        ...
        header("Content-Disposition:attachment;filename=" . $exam_info["exam_name"] . ".xls");
