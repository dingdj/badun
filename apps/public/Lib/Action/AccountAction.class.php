...
class AccountAction extends Action
{
    ...
    public function authenticate(){
		...
		if(!empty($verifyInfo['user_verified_category_id'])) {
			$verifyInfo['category']['title'] = M('user_verified_category')->where('user_verified_category_id='.$verifyInfo['user_verified_category_id'])->getField('title');
		}
		...
		// 获取认证分类
		$category = M('user_verified_category')->findAll();
    ...
    public function delverify(){
    	$verified_group_id = M('user_verified')->where('uid='.$this->mid)->getField('usergroup_id');
    	$res = M('user_verified')->where('uid='.$this->mid)->delete();
    	$res2 = M('user_group_link')->where('uid='.$this->mid.' and user_group_id='.$verified_group_id)->delete();
    	if($res && $res2){
    		//清除权限组 用户组缓存
