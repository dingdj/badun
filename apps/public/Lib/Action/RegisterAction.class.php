...
class RegisterAction extends CommonAction
{
    ...
    public function doOtherStep1()
    {
        ...
        if (!$_POST['is_bind']) {
            ...
            $reg_captcha = $_POST['reg_captcha']; //验证码
            $bindemail   = model('AddonData')->get('login:bindemail');
            $password    = trim($_POST['password']);
            $repassword  = trim($_POST['repassword']);
            //检查验证码 $_SESSION['code']
            //echo $_SESSION['phoneverify'].'<br>'.$_SESSION['getverphone'];
            if (intval($reg_captcha) != $_SESSION['phoneverify'] || empty($_SESSION['getverphone']) || $_SESSION['getverphone'] != $phone) {
                $this->mzError("验证码错误！");
            }
            if ($password != $repassword) {
                $this->mzError("对不起，两次密码不一致！");
            }
            //直接绑定
            ...
            $map['ctime']      = time();
            $map['phone']      = $phone;
            $map['mhm_id']     = intval($_POST['mhm_id']) ? : 1;
            $res_phone         = M('user')->where("phone='" . $phone . "'")->count();
            $res_email         = M('user')->where("login='" . $email . "'")->count();
            if ($res_phone > 0) {
                $this->mzError("对不起，此手机已被注册，请更换！");
            }
            if ($res_email > 0) {
                $this->mzError("对不起，此邮箱已被注册，请更换！");
            }
            //        // 添加地区信息
...
//                    D('Passport')->loginLocal($email,$password);
    ...
    public function doStep1()
    {
        ...
        if ($invite) {
            $invite_result = model('Invite')->checkInviteCode($inviteCode);
            $this->assign('isAdmin', 1);
            if ($invite_result === '0') {
                $this->error("您使用了不存在的邀请码");
            } else if ($invite_result === '2') {
                $this->error("您获得的邀请码已经被使用过了");
            }
        }
        $uid = $this->_user_model->add($map);
        if ($uid) {
