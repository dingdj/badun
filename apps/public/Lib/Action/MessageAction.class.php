...
class MessageAction extends Action
{
    ...
    public function doPost() {
		...
		$dtime = date('Y-m-d H:i:s',$_POST['ctime']);
		$nowboject =$_POST['nowboject'];
...
//        $res = model('Notify')->sendMessage($_POST, $this->mid);
    ...
    public function comment(){
        $comsg=M('ZyComment')->where(array('fid'=>$this->mid,'is_del'=>0))->order("ctime DESC")->findPage(10);
        //循环格式化数据
        $ids="";
        foreach($comsg['data'] as &$val){
            $ids=$ids.$val['id'].",";
            ...
            if($val['app_table']=="wenda"){
                ...
                $val['url']=U('wenda/Index/detail',array('id'=>$val['app_id'],'commentid'=>$val['comment_id']));
            }else if($val['app_table']=="group"){
                $gid=M('GroupTopic')->where(array('id'=>$val['app_id']))->getField('gid');
                  $val['app_name']="小组评论";
                 $val['url']=U('group/Topic/topic',array('gid'=>$gid,'tid'=>$val['app_id']));
             }else if($val['app_table']=="event"){
                $uid=M('event')->where(array('id'=>$val['app_id']))->getField('uid');
                  $val['app_name']="活动评论";
                 $val['url']=U('event/Index/eventDetail',array('id'=>$val['app_id'],'uid'=>$uid));
             }else if($val['app_table']=="ZyTopic"){
                $uid=M('event')->where(array('id'=>$val['app_id']))->getField('uid');
                  $val['app_name']="资讯评论";
                 $val['url']=U('classroom/Topic/view',array('ids'=>$val['app_id']));
            }else if($val['app_table']=="crowdfunding"){
                if(!empty($val['to_comment_id'])){
                    $val['app_name']="回复我的众筹评论：";
                }else{
                    $val['app_name']="众筹评论";
                }
                 $val['url']=U('classroom/Crow/view',array('id'=>$val['app_id']));
            }
            else if($val['app_table']=="zy_topic") {
                if(!empty($val['to_comment_id'])){
                    $val['app_name']="回复我的资讯评论：";
                }else{
                    $val['app_name']="资讯评论";
                }
        ...
        $data['is_read']=1;
        D('ZyComment')->where($where)->save($data);
        $this->assign("data",$comsg);
        $this->display();
    ...
    public function delComsg(){
        $id=$_POST['id'];
        $data['is_del']=1;
        $res=M('ZyComment')->where(array('id'=>$id))->save($data);
        if($res!==false){
            echo 200;
            exit;
        }else{
