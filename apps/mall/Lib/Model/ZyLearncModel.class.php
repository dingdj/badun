...
class ZyLearncModel extends Model {
     ...
     * @return array
     */
    public function getUserVip($uid=0){
        $user = M('credit_user')->where(array('uid'=>$uid))->find();
        $time = time();
        if($user['vip_expire'] < $time){
            $user['vip_type'] = 0;
        }
        $user_vip = M('user_vip')->where(array('id'=>$user['vip_type'],'is_del'=>0))->getField('title');

        return $user_vip ? : '普通用户';
    }
}