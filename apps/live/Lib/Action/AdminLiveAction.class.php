...
class AdminLiveAction extends AdministratorAction
{
    ...
    public function _initialize() {
        ...
        $this->pageTitle['activity']   = '直播课堂未审核列表';
        ...
        $this->pageTab[] = array('title'=>'直播课堂未审核列表','tabHash'=>'activity','url'=>U('live/AdminLive/index',array('activity'=> '1')));
    ...
    public function index(){
        ...
        //搜索字段
        $this->searchKey = array('id', 'uid', 'mhm_id','video_title','v_price','t_price','video_score','is_charge','is_best','is_best_like','is_cete_floor','is_re_free',array('listingtime','listingtime2'),'quanzhong');
    ...
    public function addLive(){
        if( isset($_POST) ) {
            ...
            if(empty($_POST['mhm_id'])){$this->error("机构ID不能为空");}
            ...
            $data['mhm_id'] = intval(t($_POST['mhm_id']));
            ...
            if($_GET['crow_id']){ //众筹课程
                ...
                $this->pageKeyList   = array('crow_id','crow_name','video_title','live_cate','mhm_id','cover','video_binfo','video_intro','str_tag',
                    'video_score','v_price','t_price','listingtime','uctime','maxmannums','is_best','is_charge');
                ob_start();
                ...
                $this->pageKeyList   = array('video_title','live_cate','mhm_id','cover','video_binfo','video_intro','str_tag','video_score',
                    'v_price','t_price','listingtime','uctime','maxmannums','is_charge','is_mount','is_best','best_sort','is_best_like',
                    'best_like_sort','is_cete_floor','cete_floor_sort','is_re_free','re_free_sort');
                ...
                $this->opt['is_mount']   = array('0'=>'否','1'=>'是','2'=>'待审核');
                $this->opt['is_best_like']   = array('0'=>'否','1'=>'是');
                $this->opt['is_cete_floor']   = array('0'=>'否','1'=>'是');
                $this->opt['is_re_free']   = array('0'=>'否','1'=>'是');
                ob_start();
            ...
            $all_school = model('School')->getAllSchol($maps,'id,title');
    ...
    public function editLive(){
        if( isset($_POST) ) {
            ...
            $this->pageKeyList   = array('id','video_title','live_cate','mhm_id','cover','video_binfo','video_intro','str_tag','video_score',
                'maxmannums','v_price','t_price','listingtime','uctime','is_charge','is_mount','is_best','best_sort','is_best_like',
                'best_like_sort','is_cete_floor','cete_floor_sort','is_re_free','re_free_sort');
            $this->notEmpty = array('video_title','live_cate','mhm_id','cover','video_binfo','video_intro','maxmannums','v_price','t_price','listingtime','uctime');
            ...
            if(!empty($live_info['crow_id'])){
                $this->pageKeyList   = array('id','crow_id','crow_name','video_title','live_cate','mhm_id','cover','video_binfo','video_intro','str_tag','video_score',
                    'maxmannums','v_price','t_price','listingtime','uctime','is_mount','is_best','is_charge');
    ...
    public function zshdLiveRoom(){
        ...
        $this->pageKeyList = array('id','uname', 'subject', 'speaker_name', 'teacherToken','assistantToken', 'studentClientToken',
            'studentToken','maxAttendees','blueconnums', 'startDate', 'invalidDate', 'clientJoin', 'webJoin', 'is_active',
            'is_open', 'DOACTION');
        //搜索字段
        $this->searchKey = array('id','uname', 'subject', 'speaker_id',  ['startDate','startDate1'], 'clientJoin', 'webJoin', 'is_active','is_del');

        $this->pageButton[] = array('title' => "搜索", 'onclick' => "admin.fold('search_form')");
        $this->pageButton[] = array('title' => "新建直播课时", 'onclick' => "admin.jump('Zshd',{$live_id})");

        $this->opt['speaker_id'] = M('zy_teacher')->where('is_del=0')->getField('id,name');
        $this->opt['speaker_id'] = array('0'=>'请选择')+$this->opt['speaker_id'];
    ...
    public function ghLiveRoom(){
        ...
        $this->pageKeyList = array('id','uname','title','speaker_name','account','passwd','maxAttendees','blueconnums','startDate','invalidDate',
            'supportMobile','is_active', 'is_open','DOACTION');
        $this->searchKey = array('id','uname', 'subject', 'speaker_id',  ['startDate','invalidDate'], 'clientJoin', 'webJoin', 'is_active','is_del');

        //搜索字段
        $this->searchKey = array('id','uname', 'subject', 'speaker_id',  ['startDate','invalidDate'], 'clientJoin', 'is_active','is_del');
        $this->pageButton[] = array('title' => "搜索", 'onclick' => "admin.fold('search_form')");
        $this->pageButton[] = array('title' => "新建直播课时", 'onclick' => "admin.jump('Gh',{$live_id})");
    ...
    public function ccLiveRoom(){
        ...
        $this->pageButton[] = array('title' => "搜索", 'onclick' => "admin.fold('search_form')");
        $this->pageButton[] = array('title' => "新建直播课时", 'onclick' => "admin.jump('Cc',{$live_id})");
...
//        'teacherToken','assistantToken', 'studentClientToken',
        $this->pageKeyList = array('id','uname', 'subject', 'roomid', 'speaker_name','startDate', 'invalidDate', 'clientJoin', 'is_active','is_open', 'DOACTION');
        //搜索字段
        $this->searchKey = array('id','uname', 'subject', 'speaker_id',  ['startDate','invalidDate'], 'clientJoin', 'is_active','is_del');

        $this->pageButton[] = array('title' => "搜索", 'onclick' => "admin.fold('search_form')");
        $this->pageButton[] = array('title' => "新建直播课时", 'onclick' => "admin.jump('Wh',{$live_id})");

        $all_teacher = M('zy_teacher')->where('is_del=0')->getField('id,name') ? : [];
        $all_teacher = array('0'=>'请选择')+$all_teacher;
        $this->opt['speaker_id'] = $all_teacher;
...
//        $this->opt['webJoin']    = ['0'=>'不限','1'=>'否','2'=>'是'];
    ...
    public function addZshdLiveRoom(){
        ...
        if(isset($_POST)){
            ...
            $param = 'subject='.urlencode(t($_POST['subject'])).'&startDate='.t($startDate*1000).
                ...
                '&uiMode='.intval($_POST['uiMode']).'&speakerInfo='.urlencode(t($speaker['inro']));
            ...
            $this->pageKeyList   = array('subject','speaker_id','startDate','invalidDate','maxAttendees','uiMode','clientJoin','webJoin',
                'teacherToken','assistantToken','studentClientToken','studentToken','description' ,'scheduleInfo');
            $this->notEmpty = array('subject','speaker_id','startDate','invalidDate','maxAttendees','uiMode','clientJoin','webJoin',
                'teacherToken','assistantToken','studentClientToken','studentToken','description' ,'scheduleInfo');
    ...
    public function editZshdLiveRoom(){
        ...
        if( isset($_POST) ) {
            ...
            if($data['webJoin'] == 1 && $data['clientJoin'] == 1){
                $this->pageKeyList   = array('roomid','subject','speaker_id','startDate','invalidDate','maxAttendees','uiMode','clientJoin','webJoin','teacherToken','assistantToken','studentToken','studentClientToken','description','scheduleInfo');
            }else if($data['webJoin'] == 1){
                $this->pageKeyList   = array('roomid','subject','speaker_id','startDate','invalidDate','maxAttendees','uiMode','clientJoin','webJoin','teacherToken','assistantToken','studentToken','description','scheduleInfo');
            }else if($data['clientJoin'] == 1){
                $this->pageKeyList   = array('roomid','subject','speaker_id','startDate','invalidDate','maxAttendees','uiMode','clientJoin','webJoin','teacherToken','assistantToken','studentToken','studentClientToken','description','scheduleInfo');
            }
            $this->notEmpty = array('subject','speaker_id','startDate','invalidDate','maxAttendees','uiMode','clientJoin','webJoin','teacherToken','assistantToken','studentToken','studentClientToken','description','scheduleInfo');
            ...
            $this->savePostUrl = U('live/AdminLive/editZshdLiveRoom',['id'=>$_GET['id'],'live_id'=>$live_id]);
            $this->displayConfig($data);
        }
    }
    ...
    public function addCcLiveRoom(){
        ...
        if( isset($_POST) ) {
            ...
            $this->pageKeyList   = array('subject','speaker_id','startDate','invalidDate','maxAttendees','uiMode','clientJoin',
    ...
    public function editCcLiveRoom(){
        ...
        if( isset($_POST) ) {
            ...
            // 数据的格式化
            $live_room_data = M('zy_live_cc')->where('id='.intval($_GET['id']) )->find();
    ...
    public function addWhLiveRoom(){
        ...
        if( isset($_POST) ) {
            ...
            $speaker = M('zy_teacher')->where("id={$_POST['speaker_id']}")->field('id,name,inro')->find();
...
//
                ...
                $find_data['fields']      = "id,subject,introduction,layout";
...
//            $this->onload[] = 'admin.checkLoadCC()';
            ...
            $this->pageKeyList   = array('subject','speaker_id','startDate','invalidDate','clientJoin','uiMode','description');
            $this->notEmpty = array('subject','speaker_id','startDate','invalidDate','uiMode','webJoin','description');
...
//                $data['studentClientToken'] = $live_info_res['playPass'];
                ...
                $result = M('zy_live_thirdparty')->save($data);
...
//            $_POST['mhm_title'] && $map['mhm_id']  = model('School')->where(array('title'=>['like','%' . t($_POST['mhm_title']) . '%']))->getField('id');
        ...
        foreach($liveInfo['data'] as $key => $val){
            ...
            if($val['live_type'] == 1){
                ...
                $liveInfo['data'][$key]['DOACTION'] .= '<a href="'.U('live/AdminLive/addCcLiveRoom',array('id'=>$val['id'])).'">新建直播课时</a> | ';
    ...
    private function _getZshdLiveList($type,$limit,$order,$map) {
        ...
        foreach($liveInfo['data'] as $key => $val) {
            ...
            $teacher_info = M('zy_teacher')->where('id ='.$val['speaker_id'])->field('id,name')->find();
...
//            $liveInfo['data'][$key]['DOACTION'] = '<a title="查看直播课堂详细信息" href="' . U('live/Admin/checkLive', array('tabHash' => 'checkLive', 'id' => $val["id"])) . '">查看</a> | ';
    ...
    private function _getGhLiveList($type,$limit,$order,$map) {
        ...
        foreach($list['data'] as &$val){
            ...
            $teacher_info = M('zy_teacher')->where('id ='.$val['speaker_id'])->field('id,name')->find();
            $val['speaker_name'] = getQuickLink(U('classroom/Teacher/view',['id'=>$teacher_info['id']]),$teacher_info['name'],'未知讲师');
    ...
    private function _getWhLiveList($type,$limit,$order,$map) {
        ...
        foreach($list['data'] as &$val){
            ...
            $teacher_info           = M('zy_teacher')->where('id ='.$val['speaker_id'])->field('id,name')->find();
            $val['speaker_name']    = getQuickLink(U('classroom/Teacher/view',['id'=>$teacher_info['id']]),$teacher_info['name'],'未知讲师');
            $val['clientJoin']      = $val['clientJoin'] ? '<p style="color: red;">非公开</p>' : '<p style="color: green;">公开</p>';
//            $val['webJoin']         = $val['webJoin'] ? '<p style="color: green;">开启</p>' : '<p style="color: red;">关闭</p>';
            $val['startDate']       = date('Y-m-d H:i' , $val['startDate']);
            $val['invalidDate']     = date('Y-m-d H:i' , $val['invalidDate']);
    ...
    public function doactionActiveLiveRoom(){
        ...
        $arrangedata['is_activity'] =1;

        if($type == 1)
        {
            $table = M('zy_live_zshd');
        ...
        $table ->startTrans();
        $res = $table ->where($map)->save($data);

        $arrangemap['live_id'] = $id;
    ...
    public function doactionDelLiveRoom(){
        ...
        $gh_map = array('id' => $id,);
        $zshd_map = array('id' => $id,);

        if($type == 1){
            $url   = $this->zshd_config['api_url'].'/room/deleted?';
            $roomid = M('zy_live_zshd')->where($zshd_map)->getField('roomid');
            $param = 'roomId='.$roomid;
            ...
            $roomid = M('zy_live_cc')->where('id='.intval($_POST['id']))->getField('roomid');
            ...
            if($res['result'] == 'OK') {
                $result = M('zy_live_cc')->where('id='.intval($_POST['id']))->delete();
                if($result){
                    $this->ajaxReturn(null,'彻底删除成功',1);
                } else {
                    $this->ajaxReturn(null,'彻底删除失败',0);
            ...
            $roomid = M('zy_live_thirdparty')->where('id='.intval($_POST['id']))->getField('roomid');
            ...
            if($res->code == 200) {
                $result = M('zy_live_thirdparty')->where('id='.intval($_POST['id']))->delete();
                if($result){
                    $this->ajaxReturn(null,'彻底删除成功',1);
                } else {
                    $this->ajaxReturn(null,'彻底删除失败',0);
