    ...
    private function _initTabSpecial() {
        ...
        if(!is_admin($this->mid)){
            $this->pageTab [] = array (
                'title' => '添加课程实体卡',
                'tabHash' => 'addCoupon',
                'url' => U ( 'school/AdminEntityCard/addCoupon' )
            );
        }
    }
...
//            $val['school_title'] = model('School')->where(array('id'=>$val['sid']))->getField('title');
            ...
            if($val['is_del'] == 1) {
                $val['DOACTION'] ='<a href="javascript:admin.mzCouponCardEdit('.$val['id'].',\'closeCoupon\',\'启用\',\'课程实体卡\');">启用</a>';
            }else {
                $val['DOACTION'] ='<a href="javascript:admin.mzCouponCardEdit('.$val['id'].',\'closeCoupon\',\'禁用\',\'课程实体卡\');">禁用</a>';
            }
    ...
    public function addCoupon(){
        ...
        $school = model('school')->where(array('id'=>$this->school_id))->field('id,title')->find();
        ...
        if($id){
            ...
            $this->assign('id',$id);
            $this->assign('pageTitle','编辑课程实体卡');
            $this->assign('coupon',$coupon);
            $this->assign('video',$video);
        }else{
            $this->assign('pageTitle','添加课程实体卡');
        }
        $this->display();
    }
    ...
    public function getVideoInfoList(){
        ...
        if($video_type == 3){
            $where = "status=1 AND is_mount=1";
            if($mhm_id == 1){
                $where .= " AND (mhm_id=$mhm_id OR mhm_id=0)";
            }else{
            ...
            $time = time();
            $where = "is_del=0 AND is_mount = 1 AND is_activity=1 AND uctime>$time AND listingtime<$time AND type=$video_type";
            if($mhm_id == 1){
                $where .= " AND (mhm_id=$mhm_id OR mhm_id=0)";
            }else{
                $where .= " AND mhm_id=$mhm_id";
