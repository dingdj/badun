...
class AdminLiveAction extends AdministratorAction
{
    ...
    public function _initialize() {
        ...
        $this->pageTitle['mount']   = '机构挂载直播列表';
        $this->pageTitle['closeLive']   = '直播课堂列表';
        $this->pageTitle['addLive'] = '创建直播课堂';

        $this->pageTab[] = array('title'=>'直播课堂列表','tabHash'=>'index','url'=>U('school/AdminLive/index'));
        $this->pageTab[] = array('title'=>'直播课堂未审核列表','tabHash'=>'activity','url'=>U('school/AdminLive/index',array('activity'=> '1')));
        $this->pageTab[] = array('title'=>'机构挂载直播列表','tabHash'=>'mount','url'=>U('school/AdminLive/mount'));
        $this->pageTab[] = array('title'=>'直播课堂回收列表','tabHash'=>'closeLive','url'=>U('school/AdminLive/closeLive'));
        $this->pageTab[] = array('title'=>'创建直播课堂','tabHash'=>'addLive','url'=>U('school/AdminLive/addLive'));
    ...
    public function index(){
        ...
        if(is_admin($this->mid)){
            $this->searchKey = array('id','video_title','mhm_title','v_price','t_price','video_score','is_charge',array('listingtime','listingtime2'));
        }else{
            $this->searchKey = array('id','video_title','v_price','t_price','video_score','is_charge',array('listingtime','listingtime2'));
        }
        $this->pageButton[] = array('title' => "搜索", 'onclick' => "admin.fold('search_form')");
        //$this->pageButton[] = array('title' => "去排课", 'onclick' => "admin.gotoarrcourse()");
    ...
    public function closeLive(){
        ...
        $this->searchPostUrl = U('live/AdminLive/closeLive');
    ...
    public function addLive(){
        if( isset($_POST) ) {
            ...
            if($_GET['crow_id']){ //众筹课程
                ...
                $this->pageKeyList   = array('video_title','live_cate','cover','video_binfo','video_intro','video_score','v_price','t_price','listingtime','uctime','maxmannums','is_charge','is_mount');
...
//            }
            ...
            $this->pageKeyList   = array('id','video_title','live_cate','mhm_id','cover','video_binfo','video_intro','video_score','maxmannums','v_price','t_price','listingtime','uctime','is_charge','is_mount');
            ...
            ob_end_clean();
            $live_info['live_cate'] = $output;
            if($live_info['is_mount']){
                $this->opt['is_mount']   = array('2'=>'待审核');
            }else{
                $this->opt['is_mount']   = array('0'=>'否','1'=>'是');
            }
...
//        $this->opt['webJoin']    = ['0'=>'不限','1'=>'否','2'=>'是'];
    ...
    public function addZshdLiveRoom(){
        ...
        if(isset($_POST)){
            ...
            $this->pageTitle['addZshdLiveRoom'] = $liveInfo[0]['video_title'].' 直播课堂—新建直播课时';
    ...
    public function addCcLiveRoom(){
        ...
        if( isset($_POST) ) {
            ...
            $this->pageTitle['addCcLiveRoom'] = $liveInfo[0]['video_title'].' 直播课堂—新建直播课时';
    ...
    private function _getZshdLiveList($type,$limit,$order,$map) {
        ...
        foreach($liveInfo['data'] as $key => $val) {
            ...
            if ($val['is_active'] == 1) {
                ...
                $liveInfo['data'][$key]['is_active'] = '<span title="审核此直播间" href="javascript:;"  style="color: red;">未审核</span> ';
            }
...
//            $liveInfo['data'][$key]['DOACTION'] .= " | <a target='_blank' href='javascript:;'>回放观看</a>";
            if ($val['is_del'] == 1){
    ...
    private function _getGhLiveList($type,$limit,$order,$map) {
        ...
        foreach($list['data'] as &$val){
            ...
            if ($val['is_active'] == 1) {
                ...
                $val['is_active'] = '<span title="审核此直播间" href="javascript:;" style="color: red;">未审核</span> ';
            }
    ...
    private function _getCcLiveList($type,$limit,$order,$map) {
        ...
        foreach($list['data'] as &$val){
            ...
            if ($val['is_active'] == 1) {
                ...
                $val['is_active'] = '<span title="审核此直播间"  style="color: red;">未审核</span> ';
            }
            ...
            $val['DOACTION'] .= "<a target='_blank' href='{$val['']}''>回放观看</a>";
