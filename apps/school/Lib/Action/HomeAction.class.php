<?php
tsload(APPS_PATH.'/school/Lib/Action/UserAction.class.php');
class HomeAction extends UserAction{
    ...
    */
	public function getFollowlist(){
		...
		if($type == 'me'){
			...
			$map['parent_id'] = 0;
			$order = 'ctime DESC';
			$data = M('learn_record')->where('uid=9')->order($order)->findPage($limit);
			foreach($data['data'] as $key=>&$value){
				$value['note_title'] = msubstr($value['note_title'],0,15);
				$value['note_description'] = msubstr($value['note_description'],0,150);
				$value['strtime'] = friendlyDate($value['ctime']);
    ...
    */
	public function getlearnlist(){
		...
		if($type == 'me'){
			...
			foreach($data['data'] as $key=>&$value){
				$value['strtime'] = friendlyDate($value['ctime']);
				$value['video_title'] = M('zy_video')->where(array('id'=>$value['vid']))->getField('video_title');
			}
		}
        $this->assign("data",$data['data']);
    ...
    */
	public function getbuycourselist(){
		...
		$data = M('zy_order_course o')->join("`".C('DB_PREFIX')."zy_teacher_course` c on c.`course_id`=o.`course_id`")->where($where)->field($fields)->findPage($limit);
 		foreach ($data["data"] as $key => $value) {
 			$data["data"][$key]["teacher_info"]=M("zy_teacher t")->join("`".C('DB_PREFIX')."user` u on u.`uid`=t.`uid`")->where("t.uid=".$value["course_teacher"])->field("phone,email,name,reservation_count")->find();
    ...
    */
	public function getTeachercourselist(){
		$limit      = 9;
		$teacher=M("zy_teacher")->where("uid=".$this->mid)->find();
		$uid=$teacher["id"];
		//拼接字段
		$fields= 'o.`uid`,o.`teach_way`,o.`id`'; 
		...
		$data = M('zy_order_course o')->join("`".C('DB_PREFIX')."zy_teacher_course` c on c.`course_id`=o.`course_id`")->where($where)->field($fields)->findPage($limit);
 		foreach ($data["data"] as $key => $value) {
 			$data["data"][$key]["user_info"]=M("user")->where("uid=".$value["uid"])->field("phone,email,uname,sex")->find();
 			$data["data"][$key]["course_info"]=M("zy_teacher_course")->where("course_id=".$value["course_id"])->find();
 		}
		//把数据传入模板
		$this->assign('data',$data['data']);
