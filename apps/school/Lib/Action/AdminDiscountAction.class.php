...
/**
    ...
    private function _initTabSpecial() {
        ...
        $this->pageTab [] = array (
                'title' => '打折卡列表',
                'tabHash' => 'index',
                'url' => U ( 'school/AdminDiscount/index' )
        );
        if(!is_admin($this->mid)){
            $this->pageTab [] = array (
                    'title' => '添加打折卡',
                    'tabHash' => 'addTeacher',
                    'url' => U ( 'school/AdminDiscount/addDiscount' )
            );
        }
    }
...
//			$val['school_title'] = model('School')->where(array('id'=>$val['sid']))->getField('title');
            ...
            if(!is_admin($this->mid)){
                $val['DOACTION'].=" | <a href=".U('school/AdminDiscount/addDiscount',array('id'=>$val['id'],'tabHash'=>'revise')).";>修改</a>";
            }
    ...
    public function addDiscount(){
        ...
        if($id){
            $this->savePostUrl = U ( 'school/AdminDiscount/doAddDiscount','type=save&id='.$id);
            $discount = model('Coupon')->where( 'id=' .$id )->find ();
			$discount['end_time'] = date('Y-m-d H:i:s',$discount['end_time']);
            $this->assign('pageTitle','修改打折卡');
            //说明是编辑
            $this->displayConfig($discount);
        }else{
            $this->savePostUrl = U ('school/AdminDiscount/doAddDiscount','type=add');
            $this->assign('pageTitle','添加打折卡');
            //说明是添加
            $this->displayConfig();
        }
