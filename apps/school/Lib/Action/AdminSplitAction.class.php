...
class AdminSplitAction extends AdministratorAction {
    ...
    public function _initialize() {
        parent::_initialize();
        $this->pageTab[] = array('title'=>'课程分成明细列表','tabHash'=>'splitVideo','url'=>U('school/AdminSplit/splitVideo'));
        $this->pageTab[] = array('title'=>'套餐分成明细列表','tabHash'=>'splitAlbum','url'=>U('school/AdminSplit/splitAlbum'));
        $this->pageTab[] = array('title'=>'直播课程分成明细列表','tabHash'=>'splitLive','url'=>U('school/AdminSplit/splitLive'));
        $this->pageTab[] = array('title'=>'线下课程分成明细列表','tabHash'=>'splitLineClass','url'=>U('school/AdminSplit/splitLineClass'));
    }
    

    public function splitVideo(){
        // 页面具有的字段，可以移动到配置文件中！
        $this->pageKeyList = array('id','uid','vid','video_title','sid','school_sum','share_id','share_sum','ctime','ltime','note');
...
//        $this->pageButton[] = array('title' => "删除记录", 'onclick' => "admin.delSplits('zy_split_album')");
    ...
    public function splitLive(){
        // 页面具有的字段，可以移动到配置文件中！
        $this->pageKeyList = array('id','uid','lid','video_title','sid','school_sum','share_id','share_sum','ctime','ltime','note');
...
//        $this->pageButton[] = array('title' => "删除记录", 'onclick' => "admin.delSplits('zy_split_live')");
    ...
    public function splitLineClass(){
        ...
        //搜索项 搜索条件：课程ID、课程名称、课程价格、平台、平台机构、课程机构、购买机构、分享者、日期筛选
        $this->searchKey = array('uid','vid','video_title','sum','pid','oschool_uid','sid',['ltime1','ltime2']);
    ...
    public function splitExport(){
        ...
        $objActSheet->setTitle("我的{$sheet_title}卖出分成明细导出");

        //合并单元格
        $objActSheet->mergeCells('A1:K1');
        $objActSheet->setCellValue('A1', "我的{$sheet_title}卖出分成明细{$aime}导出");
        ...
        $objActSheet->setCellValue("E2", "{$sheet_title}所属机构");
        $objActSheet->setCellValue("F2", "{$sheet_title}所属机构分成");
        $objActSheet->setCellValue("G2", "分享者");
        $objActSheet->setCellValue("H2", "分享者分成");
        $objActSheet->setCellValue("I2", "生成时间");
        $objActSheet->setCellValue("J2", "最后操作时间");
        ...
        foreach ($listData as $key => $val){
            ...
            $objActSheet->setCellValue("E$k", "{$sid_uname}");
            $objActSheet->setCellValue("F$k", "￥{$val['school_sum']}");
            $objActSheet->setCellValue("G$k", "{$share_uname}");
            $objActSheet->setCellValue("H$k", "￥{$val['share_sum']}");
            $objActSheet->setCellValue("I$k", "{$ctime}");
            $objActSheet->setCellValue("J$k", "{$ltime}");
            $objActSheet->setCellValue("K$k", "{$val['note']}");
        }
        ...
        header("Content-Disposition:attachment;filename=我的{$sheet_title}卖出分成明细{$time}.xls");
