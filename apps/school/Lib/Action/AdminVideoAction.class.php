...
//        $this->pageButton[] = array("title"=>"批量审核","onclick"=>"admin.crossVideos('','crossVideos','批量审核','课程')");
        ...
        $listData = $this->_getData(20,0,['in','2,3,4'],2);
        $this->displayList($listData);
    }
    ...
    public function addVideoLib(){
        ...
        if($_POST) {
            ...
            if(getAppConfig('upload_room','basic') == 0 ) {
                ...
                $file_size = $info_res['video']['definition'][3]['filesize'];
            ...
            if(getAppConfig('upload_room','basic') == 1 ) {
                ...
                //如果上传到CC服务器
                $filename="genwoxue".rand(5,8).time();
                $this->assign("filename" , $filename);
...
//            $val['video_title'] = $video_title;
    ...
    public function addVideo(){
        ...
        if($_GET['id']){
            ...
            $this->assign("video_intro","这是课程详情。。");
    ...
    public function doAddVideo(){
        ...
        if(empty($post['video_intro'])) exit(json_encode(array('status'=>'0','info'=>"请输入课程详情介绍")));
        ...
        $data['mhm_id'] 			 = intval(is_school($this->mid)); //机构id
...
//        $data['is_best']      		 = isset($post['is_best']) ? intval($post['best_recommend']) : 0; //编辑精选
    ...
    private function _getData($limit = 20, $is_del = 0, $is_activity = 1,$mount){
        ...
        foreach ($list['data'] as &$value){
            ...
            }else if($value['is_activity'] == 2){
                $value['activity'] = '<span style="color:blue">前台提交审核</span>';
                $value['DOACTION'] .= '<a href="javascript:void();" onclick="admin.activeVideo('.$value['id'].',\'AdminVideo\',true)">提交平台审核</a> | ';
                if($value['is_activity'] == 2){
                    $value['DOACTION'] .= '<a href="javascript:void();" onclick="admin.activeVideo('.$value['id'].',\'AdminVideo\',false)">驳回</a>';
                }
            }else if($value['is_activity'] == 3){
                $value['activity'] = '<span style="color:green">平台审核中</span>';
                $value['DOACTION'] .= '<span style="color:grey">等待平台审核</span>';
            }else if($value['is_activity'] == 4){
    ...
    private function _initSchoolListAdminMenu(){
        $this->pageTab[] = array('title'=>'通过审核课程列表','tabHash'=>'index','url'=>U('school/AdminVideo/index'));
        $this->pageTab[] = array('title'=>'待审核课程列表','tabHash'=>'unauditList','url'=>U('school/AdminVideo/unauditList'));
        $this->pageTab[] = array('title'=>'机构挂载课程列表','tabHash'=>'mount','url'=>U('school/AdminVideo/mount'));
        //$this->pageTab[] = array('title'=>'前台投稿待审课程列表','tabHash'=>'forwordUnauditList','url'=>U('school/AdminVideo/forwordUnauditList'));
        $this->pageTab[] = array('title'=>'课程回收站','tabHash'=>'recycle','url'=>U('school/AdminVideo/recycle'));
        if(!is_admin($this->mid)){
            $this->pageTab[] = array('title'=>'添加课程','tabHash'=>'addVideo','url'=>U('school/AdminVideo/addVideo'));
        }
