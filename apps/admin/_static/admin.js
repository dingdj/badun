...
admin.delUser = function(id){
    ...
    var dodel ='禁用';
    $.post(U('public/Message/doPost'),{to:id,dodel:dodel,content:"111"},function(msg) {
...
//导出用户
admin.UserexportUser = function(){
    var id = admin.getChecked();
    if(id ==''){
        ui.error( L('PUBLIC_PLEASE_SELECT_USER') );return false;
    }

    location.href = U('school/AdminUser/exportUser')+'&uid='+id;
};
 ...
 * @return void
 */
admin.verify = function(id,status,isgroup,uid,$dotitle){
  //  alert(info);exit;
    if("undefined" == typeof(id) || id==''){
        id = admin.getChecked();
    }
 ...
 * @return void
 */
admin.getVerifyBox = function (id,uid,dotite) {
    if (typeof id === 'undefined') {
        return false;
    }

    ui.box.load(U('admin/User/getVerifyBox') + '&id=' + id+ '&uid=' + uid+ '&dotite=' + dotite, '驳回理由');
    return false;
};
admin.newZixun = function() {
    location.href = U('admin/Topic/newZixun');
};
admin.editZixun = function(id) {
    location.href = U('admin/Topic/editor')+'&id='+id;
};

//处理资讯f
admin.Zixuntj = function(_id,action,title,type){
    var id = ("undefined"== typeof(_id)|| _id=='') ? admin.getChecked() : _id;
    $.post(U('admin/Topic/'+action),{id:id},function(msg){
        admin.ajaxReload(msg);
    },'json');
};
...
admin.delZixun=function(id){
    ...
    $.post(U('admin/Topic/delTopics'),{id:id},function(msg){
        admin.ajaxReload(msg);
    },'json');
};
...
admin.delSingle = function(id){
    if(confirm('确定删除此单页吗？')){
        $.post(U('admin/Single/del'), {ids:id}, function(msg){
              admin.ajaxReload(msg);
        },'json');
    }
};

admin.delSingles = function(id){
...
admin.delLoginRecord = function(){
    ...
    ids = ("undefined"== typeof(ids)|| ids=='') ? admin.getChecked() : ids;
    if(ids==''){
        ui.error('请选择你要删除的登录日志!');
        return false;
    }
    if(!confirm("确定删除选中的登录日志？")){
        return false;
    }
...
//处理咨询
admin.AskComment = function(_id,action,title,type){
    var id = ("undefined"== typeof(_id)|| _id=='') ? admin.getChecked() : _id;
    if(id==''){
        ui.error(L('id为空',{'title':title,'type':type}));
        return false;
    }
    if(confirm(L('是否执行此操作',{'title':title,'type':type}))){
        $.post(U('admin/Doubt/'+action),{id:id},function(msg){
            admin.ajaxReload(msg);
        },'json');
    }
};
...
//添加标签
admin.newTag = function() {
    location.href = U('admin/Home/newTag');
};

 //处理公告
 admin.editNotice= function(id){
     location.href = U('admin/Notice/editNotice')+'&id='+id;
    };
//查询课程
admin.getVideoTitle= function(obj){
	//获取输入的值
	...
	//不是数字
		 showVideoTitle('<font color="red">课程id为纯数字</font>');
	}else{
		//查询
		$.post(U('classroom/AdminVideoCard/getVideoTitle'),{video_id:video_id},function(data){
			//	返回数据转换为对象
			...
			if(data.status == 1){
				//成功 定义数据
				 ...
				 showVideoTitle(video_title);				 
			}else{
				 video_title = data.message;
				 showVideoTitle('<font color="red">'+video_title+'</font>');
			}
		},'json');
	}
	function showVideoTitle(video_title){
		//显示数据
		if($("#show_video_name")[0]){
			$("#show_video_name").html(video_title);
		}else{
			obj.after("<span id='show_video_name'>"+video_title+'</span>');
		}
	}
}
 ...
 * @param form
 */
admin.checkDivideIntoConfig = function (form) {
    if(form.platform_and_school.value.replace(/^ +| +$/g,'')==''){
        ui.error('平台与机构分成比例不能为空!');
        return false;
    }
    if(form.school_self_support.value.replace(/^ +| +$/g,'')==''){
        ui.error('平台与机构自营分成比例不能为空!');
        return false;
    }
    if(form.share_platform.value.replace(/^ +| +$/g,'')==''){
        ui.error('分享者与机构分成比例不能为空!');
        return false;
    }
    if(form.ouschool_and_theschool.value.replace(/^ +| +$/g,'')==''){
        ui.error('购买用户机构与课程所属机构分成比例不能为空!');
        return false;
    }
    var pac = form.platform_and_school.value;
    var platform_and_school = pac.split(":");
    if(parseFloat(platform_and_school[0]) + parseFloat(platform_and_school[1]) != 1){
        ui.error('平台与机构分成比例之和须为1');
        return false;
    }
    var pss = form.school_self_support.value;
    var school_self_support = pss.split(":");
    if(parseFloat(school_self_support[0]) + parseFloat(school_self_support[1]) != 1){
        ui.error('平台与机构自营分成比例之和须为1');
        return false;
    }
    var sp = form.share_platform.value;
    var share_platform = sp.split(":");
    if(parseFloat(share_platform[0]) + parseFloat(share_platform[1]) != 1){
        ui.error('分享者与机构分成比例之和须为1');
        return false;
    }
    var osts = form.ouschool_and_theschool.value;
    var ouschool_and_theschool = osts.split(":");
    if(parseFloat(ouschool_and_theschool[0]) + parseFloat(ouschool_and_theschool[1]) != 1){
        ui.error('购买用户机构与课程所属机构分成比例之和须为1');
        return false;
    }
    return true;
}
//批量禁用关键字
admin.delOptSK = function(action){
    var ids=admin.getChecked();
    ids = ("undefined"== typeof(ids)|| ids=='') ? admin.getChecked() : ids;
    if(ids==''){
        ui.error("参数错误，请选择要进行的操作");
        return false;
    }
    if(!confirm("是否执行此操作？")){
        return false;
    }
    $.post(U('admin/SearchKeywords/'+action),{ids:ids},function(msg){
        admin.ajaxReload(msg);
    },'json');
}
//处理搜索关键字
admin.optSK = function(_id,type){
    if(_id==''){
        ui.error('参数错误，请选择要进行的操作');
        return false;
    }
    if(confirm('是否执行此操作?')){
        $.post(U('admin/SearchKeywords/operationSk'),{id:_id,type:type},function(msg){
            admin.ajaxReload(msg);
        },'json');
    }
}
