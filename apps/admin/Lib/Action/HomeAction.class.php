...
class HomeAction extends AdministratorAction {
    ...
    public function _initialize() {
        ...
        $this->pageTitle['studentActive']= '学员活跃统计';
        $this->pageTitle['vipOrder']  	 = '订单收益统计';
        $this->pageTitle['usersOrder']	 = '注册下单统计';
    ...
    public function statistics()
    {
        ...
        $serverInfo[L('PUBLIC_CORE_VERSION')] = "{$this->site['site_keyword']} 3.0";
    ...
    public function showDataCount() {
        ...
        $list = model('Online')->getDataCount($where);
    ...
    public function studentActive() {
        ...
        $dataCount = model('Online')->getStudentCount($where);
    ...
    public function vipOrder() {
	    ...
	    //获取的统计数据
        $dataCount = model('Online')->getVipCount($where);
    ...
    public function usersOrder() {
        ...
        //获取的统计数据
        $dataCount = model('Online')->getRegCount($where);
        $this->assign('dataCount',$dataCount);
        $this->display();
    }
    ...
    public function allOrder() {
        ...
        //获取的统计数据
        $dataCount = model('Online')->getOrderCount($where);
        $this->assign('dataCount',$dataCount['data']);
        $this->display();
    }
    ...
    public function _delLogs() {
        ...
        !is_array($_REQUEST['id']) && $_REQUEST['id'] = array($_REQUEST['id']);

        LogRecord('admin_system', 'dellog', array('nums'=>count($_REQUEST['id']), 'ids'=>implode(',',$_REQUEST['id'])), true);
        exit(json_encode($return));
    }
    ...
    {
        $this->pageTitle ['newTag'] = '添加标签';
        $this->pageKeyList   = array('tag_name', 'table');
        $this->opt['table'] = model('Tag')->getTableHash();
            $this->savePostUrl = U('admin/Home/doNewTag');
            $this->displayConfig();
        }
    ...
    public function doNewTag() {
        if (! $_POST ['tag_name']) {
            ...
            $ary ['name'] = t ( $_POST ['tag_name'] );
            $app ['table'] = t ($_POST ['table'] );
            $app ['app'] =  "classroom";
            if (M('tag')-> add ($ary)) {
                $app['tag_id'] =M('tag')->where(array('name'=> $ary['name']))->getField('tag_id');
              M('app_tag')-> add($app);
                $this->assign ( 'jumpUrl', U ('admin/Home/tag') );
                $this->success ( '添加成功' );
            } else {
                $this->error ( '未知错误' );
            }
