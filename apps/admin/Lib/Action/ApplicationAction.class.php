...
class ApplicationAction extends AdministratorAction
{
    public function _initialize() {
        parent::_initialize ();
        $this->pageTitle ['index']     = '轮播图管理';
        $this->pageTitle ['addSlide']  = '添加轮播图';
        $this->pageTitle ['appConfig'] = 'APP下载配置';
        $this->pageTitle ['socket']    = 'socket配置';
        $this->pageTitle ['jpush']     = '激光推送配置';
        $this->pageTitle ['about']     = '关于我们配置';
        $this->pageTitle ['feedback']  = '反馈管理';
    }
    ...
    public function doSlide()
    {
        ...
        if (!in_array($type, array('false', 'url'))) {
            $this->error('跳转类型不正确');
        } elseif (!$title) {
            $this->error('标题不能为空');
        } elseif (!$image) {
            $this->error('必须上传轮播图片');
        } elseif (in_array($type, array('url') and !$data)) {
