...
class ContentAction extends AdministratorAction
{
	...
	public function message ($isRec = 0) {
		...
		if($isRec == 0) {
			$this->pageButton[] = array('title'=>L('删除'),'onclick'=>"admin.ContentEdit('','delMessage','".L('PUBLIC_STREAM_DELETE')."','".L('PUBLIC_PRIVATE_MESSAGE')."');");
		} else {
		...
		!empty($_POST['mix_man']) && $map['c.member_uid'] = intval(t($_POST['mix_man']));
