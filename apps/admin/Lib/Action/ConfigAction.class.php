...
class ConfigAction extends AdministratorAction {
	...
	public function site() {
		...
		ob_end_clean();
		$detailData['site_default_areahidden'] = $output;
		...
		$foote = M('system_data')->where("list='admin_config' AND `key`='site'")->find();
		$foote['value'] = unserialize($foote['value']);
...
//			'alipay_key',           //安全检验码，以数字和字母组成的32位字符
	...
	public function wxpay() {
		/*公众平台用于网页支付，开放平台用于app支付*/
		$this->pageKeyList = array (
            'mp_appid',//公众平台 绑定支付的APPID（必须配置，开户邮件中可查看）
            'mp_mchid',//公众平台 商户号
            'mp_api_key',//公众平台 微信商户API秘钥
            'mp_api_secret',//公众平台 微信公众号secret
            'mp_apiclient_cert',//开放平台 私钥
            'mp_apiclient_key',//开放平台 公钥
            'mp_api_resource',//公众平台 支付授权目录

			'APPID',       //开放平台 绑定支付的APPID（必须配置，开户邮件中可查看）
			'MCHID',       //开放平台 商户号（必须配置，开户邮件中可查看）
			'KEY',         //开放平台 商户支付密钥，参考开户邮件设置（必须配置，登录商户平台自行设置）
        ...
        $pageKeyData['mp_api_resource'] = "http://{$_SERVER['HTTP_HOST']}/my/ <br/>http://{$_SERVER['HTTP_HOST']}/pay/";
//		$this->savePostUrl = U('admin/Config/saveWxConfig');
		$this->displayConfig ($pageKeyData);
	}
	...
	public function saveWxConfig(){
		$apiclient_cert = implode(DIRECTORY_SEPARATOR, array(SITE_PATH, 'api', 'pay', 'wxpay','cert' ,'apiclient_cert.pem'));
		$apiclient_key = implode(DIRECTORY_SEPARATOR, array(SITE_PATH, 'api', 'pay', 'wxpay','cert' ,'apiclient_key.pem'));
		file_put_contents($apiclient_cert,$_POST['apiclient_cert']);
		file_put_contents($apiclient_key,$_POST['apiclient_key']);
		$config_path = implode(DIRECTORY_SEPARATOR, array(SITE_PATH, 'api', 'pay', 'wxpay','lib' ,'WxPay.Config.php'));
		$wxconfig = file_get_contents($config_path);
		$wxconfig = preg_replace('/const APPID = \'(.+)\'/i','const APPID = \''.$_POST['APPID'].'\'',$wxconfig);
		$wxconfig =preg_replace('/const MCHID = \'(.+)\'/i','const MCHID = \''.$_POST['MCHID'].'\'',$wxconfig);
		$wxconfig =preg_replace('/const KEY = \'(.+)\'/i','const KEY = \''.$_POST['KEY'].'\'',$wxconfig);
		$wxconfig =preg_replace('/const APPSECRET = \'(.+)\'/i','const APPSECRET = \''.$_POST['APPSECRET'].'\'',$wxconfig);
		file_put_contents($config_path,$wxconfig);
		$this->saveConfigData();
	}
...
//		);
		$this->pageTab[] = array(
			'title' =>'已邀请用户列表',
			'tabHash' => 'getInviteList',
			'url' => U('admin/Config/getInviteList')
		);
	}
	/**
	 * 游客配置
	...
	public function getInviteList(){
		$_REQUEST ['tabHash'] = 'getInviteList';
		$this->_initTabInvite ();
		$this->allSelected = false;
		...
		if ($_REQUEST ['dosearch'] == 1) {
			...
			if($_REQUEST['inviter_user']){
				$map['c.inviter_uid'] = intval($_REQUEST['inviter_user']);
			}
			if($_REQUEST['ctime']){
				$map['c.ctime'] = array('BETWEEN',strtotime($_REQUEST['ctime']).",".strtotime($_REQUEST['endtime']));
			}
		}
		$listData = model ( 'Invite' )->getInviteList ( $type,$map );
		foreach ( $listData ['data'] as $key => &$value ) {
			$value ['face'] = '<img src="' . $value ['avatar_small'] . '" />';
			$receiverInfo = model ( 'User' )->getUserInfo ( $value ['receiver_uid'] );
			$value ['receiver_uname'] = $receiverInfo ['uname'];
	...
	public function unionpay() {
		...
		$security_key = M('system_data')->where("`list`='admin_Config' AND `key`='unionpay'")->field('value')->find();
		$this->displayConfig ();
	}
	...
	public function doaudit() {
		...
		if (model ( 'Xdata' )->put ( 'keywordConfig', $data ) && model ( 'Xdata' )->put ( 'keyword_replaceConfig', $replace )) {
...
" . $_POST ['ucenter_config'];
	 ...
	 *
	 */
	public function divideIntoConfig(){
		$this->pageKeyList = array (
			//平台与机构
			'platform_and_school',
			//平台与机构自营
			...
			'share_platform',
			//购买用户机构与课程所属机构
			'school_and_buyschool',
//			'share_use_day',//分享者分成收人使用期限
//			'share_divito_num',//分析者从购买者获取分成的次数
			'ouschool_buyer_day',//网校从自己的用户 购买者获取分成的有效期限
			'ouschool_buyer_num',//网校从自己的用户 购买者获取分成的次数
		);
		$this->notEmpty = array('platform_and_school','school_self_support','share_platform','ouschool_and_theschool');
...
//        $this->onsubmit = 'admin.checkDivideIntoConfig(this)';
	...
	public function setIndex(){
		...
		$this->display('set_index');
	}

    /**
     * 评论点评开关设置
     */
    public function commentSwitch(){
        $this->pageKeyList = array(
            'course_switch',
            'live_switch',
            'group_topic_switch',
        ...
        $this->opt ['course_switch'] = array('1' => '关闭评论', '0' => '开启评论');
        $this->opt ['live_switch'] = array('1' => '关闭评论', '0' => '开启评论');
        $this->opt ['group_topic_switch'] = array('1' => '关闭评论', '0' => '开启评论');
        $this->opt ['news_switch'] = array('1' => '关闭评论', '0' => '开启评论');
        $this->opt ['album_switch'] = array('1' => '关闭评论', '0' => '开启评论');
        $this->opt ['wenda_switch'] = array('1' => '关闭评论', '0' => '开启评论');
