...
class AdministratorAction extends Action {
    ...
    */
     ...
     * @return string HTML页面数据
     */
    public function displayCoverTree($tree = array(), $stable = null, $level = 0, $delParam = null, $extra = '', $limit = 0)
    {
        $this->assign('stable', $stable);
        if(!isset($delParam['module']) || !isset($delParam['method'])) {
            $delParam = null;
        ...
        $this->assign('extra', $extra);
        $this->assign('limit', $limit);
        $this->display(THEME_PATH.'/admin_cover_tree.html');
    }
     ...
     * @return string HTML页面数据
     */
    public function displayAree($tree = array(), $stable = null, $level = 0, $delParam = null, $extra = '', $limit = 0)
    {
        $this->assign('stable', $stable);
        if(!isset($delParam['module']) || !isset($delParam['method'])) {
            $delParam = null;
        ...
        $this->assign('extra', $extra);
        $this->assign('limit', $limit);
        $this->display(THEME_PATH.'/admin_aree.html');
    }
    /**
     * 现实分类页面
     * @param array $tree 树形结构数据
     ...
     * @return string HTML页面数据
     */
    public function displaySree($tree = array(), $stable = null, $level = 0, $delParam = null, $extra = '', $limit = 0)
    {
        $this->assign('stable', $stable);
        if(!isset($delParam['module']) || !isset($delParam['method'])) {
            $delParam = null;
        ...
        $this->assign('extra', $extra);
        $this->assign('limit', $limit);
        $this->display(THEME_PATH.'/admin_sree.html');
    }
...
//         }
        ...
        if ( $key == 'admin_Config:attach' ){
        	...
        	//$objext = array('gif','png','jpeg','zip','rar','doc','xls','ppt','docx','xlsx','pptx','pdf','jpg','mp3','mp4','flv','f4v');
...
//        dump($key);dump(model('Xdata')->get($key));exit;//in_link
	...
	public function checkAccess(){
        $rule = "{$_GET['app']}/{$_GET['mod']}/{$_GET['act']}";
