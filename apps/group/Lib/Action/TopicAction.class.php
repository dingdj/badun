	...
	class TopicAction extends BaseAction
	{
		...
		function index() {
			...
			if ('my' == $_GET['cid']) {
				...
				$threadcount=M('group_topic')->where(array('gid'=>$_GET['gid'],'uid'=>$this->mid,'is_del'=>0))->findAll();
				$this->assign('cid', $_GET['cid']);
			} else if ('dist' == $_GET['cid']) {
				$cid = " AND dist=1 ";
				$threadcount=M('group_topic')->where(array('gid'=>$_GET['gid'],'dist'=>1,'is_del'=>0))->findAll();
				$this->assign('cid', $_GET['cid']);
			} else {
				$cid = (is_numeric($_GET['cid']) && $_GET['cid'] > 0) ? " AND cid={$_GET['cid']} " : '';
				$threadcount=M('group_topic')->where(array('gid'=>$_GET['gid'],'is_del'=>0))->findAll();
			...
			$search_key = $search_key?" AND title LIKE '%{$search_key}%' ":'';
           	$order= $_GET['order'] ? 'addtime' : 'replytime';
			$topiclist = $this->topic
							  ->order('top desc ,dist desc,'.$order.' DESC')
			...
			//小组组员
			$group_list=M('group_member')->where('level>1 and status=1 and gid='.$this->gid)->findAll();
			foreach ($group_list as $v) {
				$group_id[]=$v['uid'];
			}
			
			//推荐帖子  按阅读量获取
			$recommend_post = M('group_topic')->where('is_del=0')->order('replycount desc')->limit('5')->findALl();
			$usermap['uid']=array('in',$group_id);
			$group_list = M('user')->where($usermap)->findAll();
			$this->assign('category_list', $_category_list); // 帖子分类
			$this->assign('topiclist', $topiclist);
            $this->assign('threadcount',count($threadcount));
            $this->assign('group_list',$group_list);
			$this->assign('mid', $this->mid);
			$this->assign('recommend_post', $recommend_post);
        	$this->setTitle("小组 - " . $this->groupinfo['name']);
 			$this->display();
		}
		...
		public function doAdd()
		{
			if(isset($_POST['addsubmit']) && trim($_POST['addsubmit']) == 'do') {
				...
				if(empty($title)){$this->assign("isAdmin",1); $this->error('标题不能为空');}
				...
				$topic['title'] = filter_keyword(h(t($title)));
				$topic['cid']   = intval($_POST['cid']);
				$topic['addtime'] = time();
				$topic['replytime'] = time();
                $topic['replytime'] = time();
                $topic['intro'] = filter_keyword(h($_POST['content']));
				if($tid = D('Topic')->add($topic)) {
					$post['gid'] = $this->gid;
					$post['uid'] = $this->mid;
					$post['tid'] = $tid;
					$post['intro'] = h($_POST['content']);
					$post['istopic'] = 1;
					$post['ctime'] = time();
					$post['ip'] = get_client_ip();
					$post_id = $this->post->add($post);
                    M('group')->where(array('id'=>$this->gid))->setInc("threadcount");
					// 微博
...
//				$content = h($_POST['content']);
                $this->__checkContent($_POST['intro'], 10, 5000);
                $map['intro'] = h($_POST['intro']);
				...
				$this->post->setField('content', $intro, 'tid='.$tid." AND istopic=1");

				redirect(U('group/Topic/topic',array('gid'=>$this->gid,'tid'=>$tid)));
...
// 	    				D('GroupUserCount')->addCount($post_info['uid'], 'bbs', $this->gid);
		...
		public function editPost()
		{
			...
			//管理员或者帖子主人
			if(!$post) {   $this->assign("isAdmin",1);$this->error('帖子回复不存在');}
		...
		public function search() {
			...
			if(isset($_POST['searchSubmit'])){
				...
				if(!$keywords){  $this->assign("isAdmin",1); $this->error('关键字太少');}
		...
		public function topic()
		{
			...
			$gid = intval($_GET['gid']) > 0 ? $_GET['gid'] : 0;

            $membermap['gid'] =  $gid;
            $membermap['uid'] =  $this ->mid;
            $level =  M('group_member')->where($membermap)->getField('level');

			if($tid == 0){  $this->assign("isAdmin",1);$this->error('参数错误');}
			$limit = 20;
			...
			//其他帖子
			$otherThread = M('group_topic')->field('id,gid,title')->where(array('uid'=>$thread['uid'],'is_del'=>0))->limit(5)->findAll();
			//最新帖子
			$hotThread = M('group_topic')->limit(5)->where(array('is_del'=>0))->field('id,gid,title')->order('addtime desc')->findAll();


			$commentSwitch = model('Xdata')->get('admin_Config:commentSwitch');
			$switch = $commentSwitch['group_topic_switch'];

            $this->assign('topic', $thread);
			$this->assign('tid', $tid);
			$this->assign('switch', $switch);
			$this->assign('postlist', $postlist);
			$this->assign('hotThread', $hotThread);
			$this->assign('isCollect', D('Collect')->isCollect($tid,$this->mid));  //判断是否收藏
			$this->assign('otherThread', $otherThread);
...
// 	    				D('GroupUserCount')->addCount($v['uid'], 'bbs', $this->gid);
				...
				$credit = M('credit_setting')->where(array('id'=>25,'is_open'=>1))->field('id,name,score,count')->find();
				if($credit['score'] > 0){
					$ttype = 6;
					$note = '被设置为精华帖子获得的积分';
				}
				...
				foreach($topic as &$v){
                    model('Credit')->addUserCreditRule($v['uid'],$ttype,$credit['id'],$credit['name'],$credit['score'],$credit['count'],$note);
                }
...
// 	    				D('GroupUserCount')->addCount($v['uid'], 'bbs', $this->gid);
				...
				$credit = M('credit_setting')->where(array('id'=>26,'is_open'=>1))->field('id,name,score,count')->find();
				if($credit['score'] > 0){
					$ttype = 6;
					$note = '帖子被置顶获得的积分';
				}
				...
				if ($_POST['ajax'] == 1) {
					if ($res === false) {
                        ...
                        if (strpos($id, ',')) {
                            $map['id'] = array('IN',$id);
                            $map['gid'] = $this->gid;
                            $topic = $this->topic->field('uid')->where($map)->findAll();
                        } else if (is_numeric($id)) {
                            $map['id']  = $id;
                            $map['gid'] = $this->gid;
                            $topic = $this->topic->field('uid')->where($map)->findAll();
                        }
                        $credit = M('credit_setting')->where(array('id'=>45,'is_open'=>1))->field('id,name,score,count')->find();
                        if($credit['score'] < 0){
                            $ttype = 7;
                            $note = '删除帖子扣除的积分';
                        }
                        if(count($topic) == count($topic,1)){
                            $topicInfo[] = $topic;
                        }else{
                            $topicInfo = $topic;
                        }
                        foreach($topicInfo as &$val){
                            model('Credit')->addUserCreditRule($val['uid'],$ttype,$credit['id'],$credit['name'],$credit['score'],$credit['count'],$note);
                        }
